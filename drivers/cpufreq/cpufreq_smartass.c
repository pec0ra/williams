 ...
 */
#define DEFAULT_AWAKE_MIN_FREQ 38400
...
static void cpufreq_smartass_timer(unsigned long data)
{
        ...
        if ((cpu_load > max_cpu_load || delta_idle == 0) &&
            !(policy->cur > this_smartass->max_speed &&
              (update_time - this_smartass->freq_change_time) > 100*down_rate_us)) {
                ...
                if ((update_time - this_smartass->freq_change_time) < up_rate_us)
                        return;
	...
	if (!cpumask_test_cpu(smp_processor_id(), policy->cpus))
         ...
         * minimum sample time.
         */
        if ((update_time - this_smartass->freq_change_time) < down_rate_us)
                return;
